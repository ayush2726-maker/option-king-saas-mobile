...
function BiometricAppLock({ children }) {
  ...
  const authenticate = React.useCallback(async () => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        setLocked(true);
        setMessage("Unlock cancel hua. Fingerprint/Face ID dobara try karo ya password login use karo.");
      }
    } catch (error) {
      if (!mountedRef.current) return;
      setLocked(true);
      setMessage("Biometric unlock available nahi hua. Dobara try karo ya password login use karo.");
    } finally {
  ...
  const evaluateSession = React.useCallback(async ({ prompt = false } = {}) => {
    try {
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        "Trading dashboard kholne ke liye fingerprint, Face ID, ya device security use karo."
      ),
      message
      ...
      React.createElement(
        Text,
        { style: styles.note },
        "Password login choose karne par saved session remove hogi; account data delete nahi hoga."
      )
    )
  );
}
...
module.exports = BiometricAppLock;
module.exports.BIOMETRIC_LOCK_MARKER = "OKAI_BIOMETRIC_APP_LOCK_V1";
